...
We are making use of the `@langchain_factory` decorator to tell Chainlit on how to get the LangChain agent.
Then, we use `@langchain_postprocess` (which takes the raw response from the agent) as well as `send_text_document` to:

...
If you are using the [Local Persistence](/cloud/persistence/getting-started) mode, you will need to run the following command to update the database schema:
...
This example can be applied to any framework. Simply create a per-user instance in [on_chat_start](/api-reference/on-chat-start), store it in the [user_session](/backend/user-session), and retrieve it in [on_message](/api-reference/on-message) to process the message.

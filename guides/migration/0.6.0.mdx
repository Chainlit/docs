...
This example can be applied to any framework. Simply create a per-user instance in [on_chat_start](/api-reference/on-chat-start), store it in the [user_session](/backed/user-session), and retrieve it in [on_message](/api-reference/on-message) to process the message.

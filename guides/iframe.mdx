...
# Embedding Chatbot Interface Using `<iframe>`

This document outlines the steps and specifications for embedding the external Chatbot UI, provided by Chainlit, into our existing frontend service. This integration is achieved using an HTML `<iframe>`. Below we detail the properties and considerations that need attention.

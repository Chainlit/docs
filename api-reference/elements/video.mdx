...
<ParamField path="display" type="ElementDisplay" optional>
  Determines where the element should be displayed in the UI. Choices are
...
@cl.on_chat_start
async def main():
    await cl.Video(name="example.mp4", path="./example.mp4", display="side").send()
    await cl.Message(
        content="Here is the example.mp4 video file",
    ).send()
```

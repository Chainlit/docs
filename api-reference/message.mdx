...
<ParamField path="command" type="str" optional>
  The [command](/concepts/command) selected by the user for this message. Set automatically by the UI when the user picks a command. Access this value in the [on_message](/api-reference/lifecycle-hooks/on-message) handler via `msg.command`.
...
<ParamField path="modes" type="Dict[str, str]" optional>
  The [modes](/concepts/modes) selected by the user for this message, as a dict of `{mode_id: option_id}`. Set automatically by the UI when the user configures mode pickers. Access this value in the [on_message](/api-reference/lifecycle-hooks/on-message) handler via `msg.modes`.

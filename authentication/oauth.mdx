...
<Warning>
  ...
  set the `base_url` field in your `.chainlit/config.toml` file under the `UI`
  category. For instance, if you host your application at
  `https://mydomain.com`, the `base_url` should be set to
  `https://mydomain.com`.
</Warning>
...
Additionally, if your application supports "Accounts in this organizational directory only" (Single tenant), you will need to explicitly set: `OAUTH_AZURE_AD_ENABLE_SINGLE_TENANT=true`.

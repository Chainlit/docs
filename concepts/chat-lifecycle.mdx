...
the `cl.on_settings_update` decorator is used to define a hook that is called when the user updates the chat settings of the app. This can only happen if the [chat settings](/chat-settings) are enabled.

```python
@cl.on_settings_update
async def setup_agent(new_settings):
    print("The user updated the chat settings!")
```
...
The `@cl.on_chat_resume` decorator is used to define a hook that is called when a user resumes a chat session that was previously disconnected. This can only happen if [authentication](/authentication) and [data persistence](/data-persistence) are enabled.

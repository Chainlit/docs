...
This function will be called every time a user inputs a message in the chatbot UI. You can put your custom logic within the function to process the user's input, such as analyzing the text, calling an API, or computing a result.

The [cl.send_message()](/api-reference/send-message) function is responsible for sending a reply back to the user. In this example, we simply send a message containing the user's input.
...
Congratulations! You've just created your first Chainlit application using pure Python. From here, you can add [elements](/concepts/elements), [actions](/concepts/actions) and [integrate with LLMs](/examples/openai-sql) to create a more sophisticated app. Happy coding! 🎉

...
Since Chainlit launched, LangChain has improved its callback handling, which mean we no longer need to patch the LangChain package.
This also means that you will need to pass the appropriate callback handler your LangChain agent as described [here](/api-reference/langchain/callbacks) and below.
...
If the agent doesn't have an async implementation, use [make_async](/api-reference/asyncify) to run the agent in a different thread.

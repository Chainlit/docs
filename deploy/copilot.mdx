...
The Copilot can also emit events to the Chainlit server. This is useful for sending context information or user actions to the Chainlit server.

First, add a `@cl.on_copilot_event` decorator to your Chainlit server:
...
async def on_copilot_event(data):
    print("Received event from copilot!")
```

The `data` parameter will contain the event data sent by the Copilot.

Then, in your app/website, you can emit an event like this:

```js
window.sendChainlitEvent({ type: "button_click" });
```

The event will be sent to the Chainlit server, and the `on_copilot_event` function will be called. You can pass any value in the event data.

...
Chainlit offers an out-of-the-box data layer schema to store your threads, steps, feedback, etc. 
To use it go to [Chainlit Data Layer](https://github.com/Chainlit/chainlit-datalayer) and run a PostgreSQL container with the specified schema. 

Then, from your Chainlit application, add the `DATABASE_URL` pointing to your PostgreSQL instance and Chainlit will
automatically store the chat history for your users, threads and feedback. 

Details to store attachments are available from the [Chainlit Data Layer](https://github.com/Chainlit/chainlit-datalayer) README.

We would love to see improvements on the data layer implementation. If you're interested in contributing, please reach out to us on Discord.

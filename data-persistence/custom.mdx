...
Import the custom data layer and storage client, and implement the `@data_layer` callback.
...
Import the custom data layer and storage client, and implement the `@data_layer` callback in your Chainlit app.
...
@cl.data_layer
def data_layer():
  return DynamoDBDataLayer(table_name="<Your Table>", storage_provider=storage_client)
```
